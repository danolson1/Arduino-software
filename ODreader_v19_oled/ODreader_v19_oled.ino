...
New in this version
-tried to fix bug where blank value is sometimes set incorrectly.  Note: this was due to bval[] character array being overwritten by ltin[] character array.  Making the character arrays longer fixed the problem
-updated displayTubeStatus function
-added a feature where briefly pressing the "calibration mode" button returns the display to summary mode
...
Pin assignments:
0: unused (used by ethernet shield in previous version)
1: unused (used by ethernet shield in previous version)
2: yellow LED
...
unsigned long connectionIntervalODRead = 700;        // delay between OD reads
...
//byte mac[] = { 0x90, 0xA2, 0xDA, 0x0D, 0x45, 0x10 }; //mac address for IODR in Montgolfier chamber
byte mac[] = { 0x90, 0xA2, 0xDA, 0x0D, 0xA7, 0x42 }; //mac address for IODR
...
void loop(){
  ...
  //note: I added a call to checkBlankButtons within the readLightSensors() function to improve responsiveness
  checkBlankButtons();
